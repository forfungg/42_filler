...
/*   Updated: 2019/12/11 14:42:10 by jnovotny         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_log_status(t_map *map, t_token *token)
{
	...
	ft_log(">>edge branch:\tedge[%d, %d]\n\t\t\t\tcrit[%d, %d]\n\t\t\t\tcr = %.3f\n", map->edge.edge.y, map->edge.edge.x, map->edge.crit.y, map->edge.crit.x, map->edge.cr);
	ft_log(">>corners: lt = %d | rt = %d | lb = %d | rb = %d\n", map->corner.lt, map->corner.rt, map->corner.lb, map->corner.rb);
